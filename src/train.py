...
def evaluate_ll(args, model, test_loader):
    ...
    np.save(os.path.join(args.fdir, 'eval_log_likelihood.npy'), per_class_ll)
...
def plot_training_curves(args, history):
    """
    Save training/validation curves: total loss, translation loss, and rotation error (deg).
    """
    if len(history) == 0:
        return
    ...
    test_loss = [d["test_loss"] for d in history]
    ...
    plt.plot(epochs, test_loss, label="Val total loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "loss_curves_total.png"), dpi=300)
    plt.close()

    has_trans = "train_trans_loss_mean" in history[0]
    has_rotdeg = "train_rot_err_deg_median" in history[0]

    # 2) Translation loss (MSE)
    if has_trans:
        train_t = [d["train_trans_loss_mean"] for d in history]
        test_t = [d["test_trans_loss_mean"] for d in history]

        plt.figure(figsize=(6, 4))
        plt.plot(epochs, train_t, label="Train translation loss")
        plt.plot(epochs, test_t, label="Val translation loss")
        plt.xlabel("Epoch")
        plt.ylabel("Translation loss (MSE)")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, "loss_curves_translation.png"), dpi=300)
        plt.close()

    # 3) Rotation error (degrees, median)
    if has_rotdeg:
        train_r = [d["train_rot_err_deg_median"] for d in history]
        test_r = [d["test_rot_err_deg_median"] for d in history]

        plt.figure(figsize=(6, 4))
        plt.plot(epochs, train_r, label="Train rotation error (median)")
        plt.plot(epochs, test_r, label="Val rotation error (median)")
        plt.xlabel("Epoch")
        plt.ylabel("Rotation error (deg)")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, "rotation_error_deg_curves.png"), dpi=300)
        plt.close()
...
def main(args):
    ...
    if os.path.exists(os.path.join(args.fdir, "checkpoint.pt")):
        ...
        if checkpoint.get('done', False):
            print("Found completed checkpoint; exiting (and evaluating).")
    ...
    for epoch in range(starting_epoch, args.num_epochs + 1):
        ...
        train_rot_err_deg_median = float(np.degrees(np.median(train_rot_errs)))
        train_rot_err_deg_mean = float(np.degrees(np.mean(train_rot_errs)))
        ...
        test_rot_err_deg_median = float(np.degrees(np.median(test_rot_errs)))
        test_rot_err_deg_mean = float(np.degrees(np.mean(test_rot_errs)))
        ...
        history.append(dict(
            ...
            test_loss=test_loss,
            train_acc_median=float(np.median(train_rot_errs)),
            test_acc_median=float(np.median(test_rot_errs)),
            train_rot_err_deg_median=train_rot_err_deg_median,
            test_rot_err_deg_median=test_rot_err_deg_median,
            train_trans_loss_mean=train_trans_loss_mean,
            test_trans_loss_mean=test_trans_loss_mean,
    ...
    # --- Save training curves as PNGs in this run's result directory ---
    try:
        plot_training_curves(args, history)
        logger.info(f"Saved training curves to {args.fdir}")
    except Exception as e:
        logger.error(f"Could not plot training curves: {e}")
...
def evaluate_speedplus_kelvins(args, model, loader):
    ...
    with torch.no_grad():
        for index, batch in enumerate(loader):
            ...
            R_pred = model.predict(img, cls).to(img.device)
        ...
        for batch in loader:
            ...
            for i in range(bsz):
                ...
                if trans_gt is not None and trans_pred is not None:
                    ...
                    if visualize:
                        out_dir = os.path.join(args.fdir, "debug_vis")
                        os.makedirs(out_dir, exist_ok=True)
                        save_path = os.path.join(out_dir, f"sample_{printed+1}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='modelnet10',
                        choices=[
                            'modelnet10',          # modelnet10 with 100 training views per instance
